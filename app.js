...
app.post('/register/', async (request, response) => {
  ...
  if (userDBDetails !== undefined) {
    ...
    if (password.length < 6) {
      ...
      response.send('Password istoo short');
    } else {
...
app.get(
  ...
  async (request, response) => {
    ...
    const userArray = likedUsers.map(eachUser => each.username);
...
app.get("/user/tweets/", authentication, async (request, response) => {
  ...
  FROM tweet LEFT JOIN reply ON tweet.tweet_id = reply.tweet_id LEFT JOIN like ON tweet_id = like.tweet_id
...
app.delete("/tweet/:tweetId/", authentication, async (request, response) => {
  const { tweetId } = request.params;
  const { userId } = request;
